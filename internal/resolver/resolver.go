							...
							if err == nil {
								...
								r.log.AddRangeError(&tsConfigSource, warnRange,
									fmt.Sprintf("Cannot read file %q: %s",
										r.PrettyPath(ast.Path{Text: fileToCheck, Namespace: "file"}), err.Error()))
							}
							found = true
							break
						}
		...
		if err != syscall.ENOENT {
			r.log.AddError(nil, ast.Loc{},
				fmt.Sprintf("Cannot read directory %q: %s",
					r.PrettyPath(ast.Path{Text: path, Namespace: "file"}), err.Error()))
		}
		return nil
	}
	info := &dirInfo{
			...
			if err != nil {
				if err == syscall.ENOENT {
					r.log.AddError(nil, ast.Loc{}, fmt.Sprintf("Cannot find tsconfig file %q",
						r.PrettyPath(ast.Path{Text: tsConfigPath, Namespace: "file"})))
				} else if err != parseErrorAlreadyLogged {
					r.log.AddError(nil, ast.Loc{},
						fmt.Sprintf("Cannot read file %q: %s",
							r.PrettyPath(ast.Path{Text: tsConfigPath, Namespace: "file"}), err.Error()))
				}
			}
		}
	}
...
func (r *resolver) parsePackageJSON(path string) *packageJson {
	...
	if err != nil {
		if err != parseErrorAlreadyLogged {
			r.log.AddError(nil, ast.Loc{},
				fmt.Sprintf("Cannot read file %q: %s",
					r.PrettyPath(ast.Path{Text: packageJsonPath, Namespace: "file"}), err.Error()))
		}
		return nil
	}
		...
		if absolute, ok := r.loadAsFile(mainPath); ok {
			...
			if err == nil {
				...
				r.log.AddRangeError(&jsonSource, mainRange,
					fmt.Sprintf("Cannot read directory %q: %s",
						r.PrettyPath(ast.Path{Text: mainPath, Namespace: "file"}), err.Error()))
			}
		}
	}
...
func (r *resolver) loadAsFile(path string) (string, bool) {
	...
	if err != nil {
		if err != syscall.ENOENT {
			r.log.AddError(nil, ast.Loc{},
				fmt.Sprintf("Cannot read directory %q: %s",
					r.PrettyPath(ast.Path{Text: dirPath, Namespace: "file"}), err.Error()))
		}
		return "", false
	}
